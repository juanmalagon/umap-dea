...
def run_simulation(params_dict: ParamsDict) -> pd.DataFrame:
    ...
    data_dict = dgp.generate_data_dict(
        n=n, N=N, M=M, alpha_1=alpha_1, gamma=gamma, sigma_u=sigma_u, verbose=False
    )
    x = data_dict["x"]
    y = data_dict["y"]
    ...
    efficiency_scores_dict = dea.calculate_dea_for_embeddings(
        embeddings_df_dict=embeddings_df_dict, y=y, rts=rts, orientation=orientation
    )
...
def export_results(
    ...
    evaluation_df.to_csv(os.path.join(results_dir, f"evaluation_df_{run_serial}.csv"), index=False)

    # Save summary
    summary_df = (
        evaluation_df.groupby(["dim_reduction_level", "dims"])
        .agg(
            {
                "mae": ["mean", "std"],
                "spearmanr": ["mean", "std"],
...
def wrapper_function(params_dict: ParamsDict, results_dir: str) -> None:
    ...
    evaluation_csv_path = os.path.join(results_dir, f"evaluation_df_{run_serial}.csv")
    ...
    if header_written:
        ...
        logger.warning("No successful evaluations — skipping summary generation.")
